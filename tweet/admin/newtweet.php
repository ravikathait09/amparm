 ...
 if(isset($_POST['addtweet'])){
		...
		if( $_POST['tweettype']=='0')
		{
				...
				if(isset($_FILES['tweet_image']) && !empty($_FILES['tweet_image']['name']))
				{
					...
					//$destination=$config['DOC_ROOT'].'/tweet_image/'.$destination;
					}
			...
			foreach($_POST['allgroup'] as $key=>$value)
			{
				...
				{
					$msg='';
						$twitterid 			= $user['tweet_user_id'];
						$oauth_token 		= $user['outh_token'];
						...
						if($_POST['tweettype'])
						{
						$array=array_reverse(explode('/',$_POST['retweet']));
						
						 $id= $array[0];
						$my_update = $connection->post('statuses/retweet', array('id' => $id));
						}else{
						if($destination){
							$image	=$config['DOC_ROOT'].'tweet_image/'.$destination;
							$tweet_img = $image	;
							$handle = fopen($tweet_img,'rb');
							$image = fread($handle,filesize($tweet_img));
							fclose($handle);
							$my_update = $connection->post('statuses/update_with_media', array('status' => $_POST['description'],'media[]'  => "{$image};Content-Type=multipart/form-data;filename={$tweet_img}"),true);
						}else{
						...
						$notification->notification="New Tweet Has been Posted ".$_POST['datetime'];
					...
					mail($user['email'], 'New Tweet has been Posted' , $msg,$headers );		
