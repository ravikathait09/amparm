...
if(isset($_POST['userid']) and isset($_POST['password'])){
	   $user  = new User();
		...
		$result=$user->findCustomRow(Array('email'=>$user->email,'password'=>$user->password));
		
		if(isset($result) and ($result['type']==0 ||$result['type']==2)){
...
?>
	...
	<form action="" method="post" name="loginForm" id="loginForm" novalidate>
		...
		<div class="footer">                                                               
			...
			<a href='register.php'  class="btn bg-olive btn-block"> Register</a>
